defmodule ElixirNoDeps.Presenter.RendererTest do
  ...
  describe "process_markdown_formatting/1" do
    test "processes headers with color styling" do
      ...
      assert String.contains?(result, "\e[96m") # bright_cyan for h1
      assert String.contains?(result, "\e[93m") # bright_yellow for h2
      assert String.contains?(result, "\e[97m") # bright_white for h3
    end
    ...
    test "processes inline code" do
      ...
      assert String.contains?(result, "\e[92m") # bright_green
    ...
    test "processes code blocks" do
      ...
      assert String.contains?(result, "\e[32m") or String.contains?(result, "\e[92m") # green or bright_green
      refute String.contains?(result, "```")
    end
    ...
    test "processes unordered lists" do
      ...
      * Item three
      ...
      assert String.contains?(result, "\e[94m") # bright_blue
    end
  ...
  describe "render_content/3" do
    test "wraps content to fit width" do
      content = "This is a very long line that should definitely be wrapped to fit within the specified width"
      result = Renderer.render_content(content, 40, 10)
      
      assert is_list(result)
      assert length(result) > 1
      # Each line should be reasonable length (considering padding)
      assert Enum.all?(result, &(String.length(&1) <= 36)) # 40 - 4 padding
    end
  ...
  describe "integration with data structures" do
    test "works with real presentation and slides" do
      ...
      assert String.contains?(processed, "\e[") # Should have ANSI codes
    end
  end
end