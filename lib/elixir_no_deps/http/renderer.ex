...
defmodule WebPageRenderer do
  @moduledoc """
  Combines HttpClient and HtmlRenderer to fetch and render web pages.
  """
  ...
  def fetch_and_render_text(url) do
    case HttpClient.get_body(url) do
      {:ok, html} ->
        {:ok, HtmlRenderer.to_text(html)}
      {:error, reason} ->
        {:error, reason}
    end
  end
  ...
  def fetch_and_render_terminal(url) do
    case HttpClient.get_body(url) do
      {:ok, html} ->
        {:ok, HtmlRenderer.to_terminal(html)}
      {:error, reason} ->
        {:error, reason}
    end
  end
  ...
  def fetch_and_summarize(url) do
    case HttpClient.get_body(url) do
      {:ok, html} ->
        summary = HtmlRenderer.summarize(html)
  ...
  def fetch_and_extract_links(url) do
    case HttpClient.get_body(url) do
      {:ok, html} ->
        {:ok, HtmlRenderer.extract_links(html)}
      {:error, reason} ->
        {:error, reason}
    end
  end
